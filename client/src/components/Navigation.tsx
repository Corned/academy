...
const NavigationItem = ({ icon, text, alignLeft = false }: INavigationItem) => {
  return (
    <div
      className="flex select-none flex-row gap-2 rounded-lg px-3 py-2 font-bold transition-all hover:cursor-pointer hover:bg-black/10"
...
const MobileNavigation = () => {
  ...
  return (
    <div className="sticky mb-auto flex w-[220px] flex-col gap-1 rounded-3xl border-2 border-[#edf1f2] p-4">
      {/* <NavigationItem icon={isOpen ? <MenuIcon /> : <XIcon />} alignLeft /> */}

      <div className="mb-5 flex cursor-pointer select-none flex-col items-center gap-2 rounded-lg p-2 text-center transition-all hover:bg-black/10">
